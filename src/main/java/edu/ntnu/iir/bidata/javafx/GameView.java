...
public class GameView implements BoardGameObserver {
  ...
  private static final List<Color> pieceColors
      = List.of(Color.RED, Color.BLUE, Color.GREEN, Color.YELLOW, Color.PURPLE);
  ...
  public GameView(BoardGame game, int boardChoice) {
    ...
    final Label title = new Label("Board Game (Grid View)");
    ...
    int cols = 10;
    final int rows = (int) Math.ceil((double) totalTiles / cols);
    ...
    replayButton.setOnAction(e -> {
      new Thread(() -> {
        try {
          ...
          javafx.application.Platform.runLater(() -> outputArea.appendText("Error replaying game: "
              + ex.getMessage() + "\n"));
          ex.printStackTrace();
        }
      }).start();
